...
import { PonerCartaCommand } from '../commands/ponerCartaComand';
...
export class ApiService {
  ...
  getMisJuegos(uid: string): Observable<JuegoModel[]> {
    return this.http.get<JuegoModel[]>(
      environment.apiBase + '/juego/listar/' + uid
    );
  }
  getAllJuegos(): Observable<JuegoModel[]> {
    return this.http.get<JuegoModel[]>(
      environment.apiBase + '/juego/listarall'
    );
  }
  getMiMazo(uid: string, juegoId: string) {
    return this.http.get(
      environment.apiBase + '/juego/mazo/' + uid + '/' + juegoId
    );
  }

  getTablero(juegoId: string): Observable<TableroModel> {
    return this.http.get<TableroModel>(
      environment.apiBase + '/juego/' + juegoId
    );
  }
  ...
  iniciarRonda(command: IniciarRondaCommand) {
    return this.http.post(
      environment.apiBase + '/juego/ronda/iniciar',
      command
    );
  }
